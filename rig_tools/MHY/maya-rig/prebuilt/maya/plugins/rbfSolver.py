...
class MatrixNN(object):
    ...
    def setIdentity(self):
        self.rc = [[float(i == j) for i in range(self.dim)]
...
class RbfSolver(OpenMayaMPx.MPxNode):
    ...
    def nodeInitializer(cls):
        ...
        cls.scale = numAttr.create(
            "scale", "sc", OpenMaya.MFnNumericData.kFloat, 1.0)
        ...
        cls.nInput = numAttr.create(
            "nInput", "ni", OpenMaya.MFnNumericData.kFloat, .0)
        ...
        cls.nKey = numAttr.create(
            "nKey", "nk", OpenMaya.MFnNumericData.kFloat, -3.402823466e38)
        numAttr.setArray(True)
        cls.mValue = numAttr.create(
            "mValue", "mv", OpenMaya.MFnNumericData.kFloat, -3.402823466e38)
        numAttr.setArray(True)
        ...
        cls.mOutput = numAttr.create(
            "mOutput", "mo", OpenMaya.MFnNumericData.kFloat, .0)
    ...
    def rebuild_solver(self, data):
        ...
        OpenMaya.MPlug(self.thisMObject(),
                       self.poses).getExistingArrayAttributeIndices(pose_ids)
...
def initializePlugin(mobject):
    ...
    try:
        mplugin.registerNode(RbfSolver.kPluginNodeName, RbfSolver.kPluginNodeId,
                             RbfSolver.nodeCreator, RbfSolver.nodeInitializer)
    except:
        OpenMaya.MGlobal.displayError(
