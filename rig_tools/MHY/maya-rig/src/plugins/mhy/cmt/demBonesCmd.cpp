...
MStatus DemBonesCmd::readMeshSequence(double startFrame, double endFrame) {
  ...
  for (int j = 0; j < model_.nB; j++) {
    ...
    // Go through the joints and check parent of each joint, if its parent
    // is also in the joint array, cache index of parent joint associated
    // with child joint index
    model_.parent(j) = -1;
