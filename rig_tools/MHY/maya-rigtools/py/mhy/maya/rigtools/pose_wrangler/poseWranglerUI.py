...
class PoseWrangler(ui_window.UIWindow, form_class):
    ...
    def __init__(self, parent=qt_util.maya_window()):
        ...
        #load rbf plugins
        majorApiVersion = int(cmds.about(apiVersion=True)/10000)
        rbf_plugin_name = 'MayaUE4RBFPlugin{}'.format(majorApiVersion)
        if not cmds.pluginInfo(rbf_plugin_name, q=True, loaded=True):
            cmds.loadPlugin(rbf_plugin_name)
